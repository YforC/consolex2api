...
| Tools 策略 | 已支持 | 后台可开启/关闭；开启时用户自带 `tools`、`tool_choice` 不被强行改写 |
| Thinking 参数 | 已支持 | 后台可设置默认思考强度，请求自带 `reasoning`、`reasoning_effort` 时优先使用请求值 |
...
| 生成参数 | Temperature、Top P、Tools 策略、默认思考强度 |
...
| `GATEWAY_TOOLS_ENABLED` | 否 | 是否启用 tools 透传和默认搜索工具，默认 `true` |
| `DEFAULT_REASONING_EFFORT` | 否 | 默认思考强度，可留空，或设置为 `low`、`medium`、`high` |
