...
| Tools 策略 | 已支持 | 后台可分别开启/关闭 `web_search` 和 `x_search` |
| Thinking 参数 | 已支持 | 默认思考强度只对 `grok-4.3` 生效，请求自带参数时优先使用请求值 |
...
| 账号管理 | 运行配置 | 登录页 |
| --- | --- | --- |
| ![账号管理](docs/images/admin-accounts.png) | ![运行配置](docs/images/admin-settings.png) | ![登录页](docs/images/admin-login.png) |
...
| `GATEWAY_TOOLS_ENABLED` | 否 | 总 Tools 开关，默认 `true`；关闭时所有 tools 都禁用 |
| `GATEWAY_WEB_SEARCH_ENABLED` | 否 | 是否允许 `web_search`，默认 `true` |
| `GATEWAY_X_SEARCH_ENABLED` | 否 | 是否允许 `x_search`，默认 `true` |
| `DEFAULT_REASONING_EFFORT` | 否 | `grok-4.3` 默认思考强度，可留空，或设置为 `low`、`medium`、`high` |
